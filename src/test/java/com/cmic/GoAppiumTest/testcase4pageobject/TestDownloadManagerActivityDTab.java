...
public class TestDownloadManagerActivityDTab extends BaseTest {
	...
	public void initCheck() {// TODO 验证比较弱
		...
		if (downloadTabPage.isElementShown(downloadTabPage.btnAllOperate)) {// 没毛病
			// 截图
		} else {
	...
	@Test(dependsOnMethods = { "checkDeleteDownloadTask" })
	...
	@Test(dependsOnMethods = { "checkDeleteTaskAndKeepTheFile" }, retryAnalyzer = FailRetry.class)
	...
	public void checkDownloadPauseAndResumeOne() throws InterruptedException {
		...
		if (!isDataStatu) {// 不是移动网络状态//WIFI
			AndroidElement e = downloadTabPage.randomGo2StartDownload();
			downloadTabAction.go2ClickAndWait(e, 0.5);
